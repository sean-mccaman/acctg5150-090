...
Three reviewer **role profiles** for validating your HW 2 answers. Each one
describes a person, a role, that you hand to an AI so it reviews your work the
way that person would.

**Why a role at all.** A plain "check my work" gives you a plain, hedged answer.
A defined role gives the AI a point of view, and an auditor looks for different
things than a manager does. Defining the reviewer is the same skill you used in
Week 1 to load context. You usually get a sharper result when you say who is
doing the work.

**How to use one.**

1. Decide which role fits. Read all three profiles and pick the one that best
...
4. Give it your work to review. Paste your Q6 through Q9 results, or your script
   and its CSV output, or reopen the session where you did the analysis.
5. Ask it to review your work as the role described above.
6. Read what it flags, decide what you agree with, and write your validation
   note.

You are not required to agree with the review. Judging whether it was right, and
whether you picked the right reviewer, is part of HW 2.

---

## Role profile 1 — Internal Auditor

Use this one to pressure-test whether your numbers are *correct*, and whether
you noticed what an auditor would notice.
...
- Whether the numbers reconcile. Journal entries balance, meaning total
  debits equal total credits, so the amounts should net to zero. If a total
  is not zero, you want to know why.
...
Use this one to check whether your analysis is *clear and usable*, meaning
whether the person who asked for it could actually act on it.
...
question you would ask them. Be direct and brief, and respond the way a
manager actually responds rather than with a long report.
```
...
Use this one to check the *mechanics*, meaning whether a tooling or data-parsing
...
produced, meaning the tools and the data handling rather than the accounting.
...
- Common parsing errors, such as stray newline characters inside a field,
  extra commas or delimiters, quoted values, and inconsistent encodings,
  anything that splits a row wrong or turns a number into text.
...
numbers, so they prove they understand how the tool produced it. Teach rather
than just grade.
```
...
You are delegating a review, but only after deciding who should do it. The
difference is between saying "the AI checked my work" and saying "I decided this
work needed an auditor's eye, set that reviewer up, and had it check my work."
The quality of the review usually comes from the role you chose and defined,
which is the same reason a vague prompt tends to produce vague output.
